        ...
        <div class="shadow overflow-hidden sm:rounded-md">
            <div class="px-4 py-5 bg-white sm:p-6">
                <div class="overflow-x-auto">
                    <form method="POST" action="{{ route('candidate.update',base64_encode($data->id)) }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mt-4">
                            ...
                            <x-jet-input id="tagline" class="block mt-1 w-full" type="text" name="tagline" value="{{$data->tagline}}" required />
                        </div>

                        <div class="mt-4">
                            <x-jet-label for="visi" value="{{ __('Visi') }}" />
                            <x-jet-input id="visi" class="block mt-1 w-full" type="text" name="visi" value="{{$data->visi}}" required />
                        </div>

                        <div class="bg-white p7 rounded w-9/12 mx-auto mt-4">
                            <x-jet-label for="photo" value="{{ __('Photo Paslon') }}" />
                            <div x-data="dataFileDnD()" class="relative flex flex-col p-4 text-gray-400 border border-gray-200 rounded">
                                <div x-ref="dnd" class="relative flex flex-col text-gray-400 border border-gray-200 border-dashed rounded cursor-pointer">
                                    <input accept="*" type="file" name="image" class="absolute inset-0 z-50 w-full h-full p-0 m-0 outline-none opacity-0 cursor-pointer" @change="addFiles($event)" @dragover="$refs.dnd.classList.add('border-blue-400'); $refs.dnd.classList.add('ring-4'); $refs.dnd.classList.add('ring-inset');" @dragleave="$refs.dnd.classList.remove('border-blue-400'); $refs.dnd.classList.remove('ring-4'); $refs.dnd.classList.remove('ring-inset');" @drop="$refs.dnd.classList.remove('border-blue-400'); $refs.dnd.classList.remove('ring-4'); $refs.dnd.classList.remove('ring-inset');" title="" />
